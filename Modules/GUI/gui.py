...
# from Modules.GUI.utils_gui import *
...
class GUI(tk.Tk):
    ...
    # Function for the search button - call to main function
    def search(self):
        self.update_progressbar(0)
        coordinates = (int(self.E_entry.get()), int(self.N_entry.get()), 36, "N")
        print(coordinates, self.Radius_entry.get())
        t = threading.Thread(target=self.run_process, args=(coordinates, self.Radius_entry.get()))
        t.start()
        progress_thread = threading.Thread(target=self.run_progressbar())
        progress_thread.start()

    def run_process(self, coordinates, radius_value):
        image, self.list_mask = get_viable_landing_in_radius(coordinates, float(radius_value), self)
        self.add_original_image(image)
        self.add_result_image(self.list_mask[-1])
        self.update_transparency(50)

    # Add check boxes for the masks
