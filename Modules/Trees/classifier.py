...
from Modules.Trees.detectree import pixel_features, pixel_response, settings, utils
...
class Classifier(object):
    ...
    def classify_img(self, img_filepath, clf, pixels_to_remove, output_filepath=None):
        """
        ...
        rows_to_remove_or_not = np.zeros(X.shape[0], dtype=bool)
        factors_matrix = np.array([img_shape[1], 1])
        rows_to_remove = pixels_to_remove@factors_matrix

        rows_to_remove_or_not[rows_to_remove] = True

        new_X = X[~rows_to_remove_or_not]

        
        

        #remove single pixels here:

        if not self.refine:
            print(new_X.shape)
            #y_pred = clf.predict(X).reshape(img_shape)
            y_res_before_shape = clf.predict(new_X)

            result_arr = np.zeros(X.shape[0],dtype=int)
            result_arr[~rows_to_remove_or_not] = y_res_before_shape
            result_arr[rows_to_remove_or_not] = 1 # 1 for marking as MARKED AS BAD
            y_pred = result_arr.reshape(img_shape)

        else:
            nontree, tree = np.hsplit(clf.predict_proba(new_X), 2)
            tree =tree.reshape(tree.shape[0])
            nontree =nontree.reshape(nontree.shape[0])

            p_nontree = np.zeros(X.shape[0],dtype=float)
            p_tree = np.zeros(X.shape[0],dtype=float)
 
            p_nontree[~rows_to_remove_or_not] = nontree
            p_tree[~rows_to_remove_or_not] = tree

            minimal_value = pow(10,-5)
            p_nontree[rows_to_remove_or_not] = minimal_value # 1 for marking as MARKED AS BAD
            p_tree[rows_to_remove_or_not] =  1-minimal_value# 1 for marking as MARKED AS BAD
